...
public class Main {
    ...
    public static void sortScoresDescendingOrder(Score[] arr) { // 삽입정렬
        for(int i=0; i<arr.length; i++) {
            for(int j=0; j<i; j++) {
                if(arr[i].scr > arr[j].scr) { // 작은사람 앞에 (선다) -> 내림차순
                    Score cur = arr[i];
                    for(int k=i; k>j; k--) // 한 칸씩 이동시 앞에서 부터 이동하면 같은값이 복사되므로 뒤에서부터 시작
                        arr[k]=arr[k-1];
                    arr[j] = cur;
                }
            }
