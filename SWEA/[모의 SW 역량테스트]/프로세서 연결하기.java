...
public class Solution_t1_1767_프로세서연결하기 {
	static int[] di={-1,1,0,0};//상0,하1,좌2,우3
	static int[] dj={0,0,-1,1};
    static int N,min,max,map[][];
    static List<int[]> list;

    static void subset(int cnt, int core, int len){
    	if(cnt==list.size()){
		    if(max<core){
		       max=core;
		       min=len;
    	...
    	for(int d=0; d<4; d++){
    		...
    		if(count==0){
    			subset(cnt+1,core+0,len+0);
    		}else{
    			ni=i; nj=j;
    			for(int c=0; c<count; c++) {
    				ni+=di[d]; nj+=dj[d]; map[ni][nj]=1;
    			}
    			subset(cnt+1,core+1,len+count);
    			ni=i; nj=j;
    			for(int c=0; c<count; c++) {
    				ni+=di[d]; nj+=dj[d]; map[ni][nj]=0;
    			}
    		}
    	}
    }
	public static void main(String[] args) throws Exception {
		System.setIn(new FileInputStream("src/res/input_t1_1767.txt"));
		BufferedReader br=new BufferedReader(new InputStreamReader(System.in));
        int T=Integer.parseInt(br.readLine().trim()); 
        ...
        for(int tc=1; tc<=T; tc++){
        	...
        	for(int i=0; i<N; i++){
        		...
        		for(int j=0; j<N; j++){
        			map[i][j]=Integer.parseInt(st.nextToken());
        			if(map[i][j]==1){
        				if(i<1||N-1<=i || j<1||N-1<=j) continue;
